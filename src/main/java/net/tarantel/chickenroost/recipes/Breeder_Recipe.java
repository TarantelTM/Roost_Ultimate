...
public record Breeder_Recipe (ResourceLocation recipeId, ItemStack output, Ingredient ingredient0, Ingredient ingredient1, Ingredient ingredient2,
                             int time) implements Recipe<Container> {
    ...
    @Override
    public boolean matches(Container pContainer, Level level) {
        if (level.isClientSide()) {
            return false;
        }
        if (ChickenRoostMod.CONFIG.BreederSeeds) {
            if (Config.ServerConfig.enablebothways.get()) {
