...
public class AChickenmelonEntity extends Chicken {
	...
	if((boolean) SpawnConfig.Spawn_Melon.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKENMELON.get(), SpawnConfig.Melon_Spawnweight.get(), SpawnConfig.Melon_MinGroupsize.get(), SpawnConfig.Melon_MaxGroupsize.get()));
	}
	}


	public AChickenmelonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKENMELON.get(), world);
	}

	public AChickenmelonEntity(EntityType<AChickenmelonEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKENMELON.get(), SpawnPlacements.Type.ON_GROUND,
