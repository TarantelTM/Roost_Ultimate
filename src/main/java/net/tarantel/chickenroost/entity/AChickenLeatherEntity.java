...
public class AChickenLeatherEntity extends Chicken {
	...
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("old_growth_birch_forest"), new ResourceLocation("forest"),
			new ResourceLocation("dark_forest"), new ResourceLocation("old_growth_pine_taiga"), new ResourceLocation("birch_forest"),
			new ResourceLocation("desert"), new ResourceLocation("old_growth_spruce_taiga"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
	if((boolean) SpawnConfig.Spawn_Leather.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKEN_LEATHER.get(), SpawnConfig.Leather_Spawnweight.get(), SpawnConfig.Leather_MinGroupsize.get(), SpawnConfig.Leather_MaxGroupsize.get()));
	}
	}


	public AChickenLeatherEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKEN_LEATHER.get(), world);
	}

	public AChickenLeatherEntity(EntityType<AChickenLeatherEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKEN_LEATHER.get(), SpawnPlacements.Type.ON_GROUND,
