...
public class AChickensnowEntity extends Chicken {
	...
	if((boolean) SpawnConfig.Spawn_Snow.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKENSNOW.get(), SpawnConfig.Snow_Spawnweight.get(), SpawnConfig.Snow_MinGroupsize.get(), SpawnConfig.Snow_MaxGroupsize.get()));
	}
	}


	public AChickensnowEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKENSNOW.get(), world);
	}

	public AChickensnowEntity(EntityType<AChickensnowEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Items.SNOWBALL));
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKENSNOW.get(), SpawnPlacements.Type.ON_GROUND,
