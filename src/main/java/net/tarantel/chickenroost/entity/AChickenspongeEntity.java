...
public class AChickenspongeEntity extends Chicken {
	...
	if((boolean) SpawnConfig.Spawn_Sponge.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKENSPONGE.get(), SpawnConfig.Sponge_Spawnweight.get(), SpawnConfig.Sponge_MinGroupsize.get(), SpawnConfig.Sponge_MaxGroupsize.get()));
	}
	}


	public AChickenspongeEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKENSPONGE.get(), world);
	}

	public AChickenspongeEntity(EntityType<AChickenspongeEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Blocks.SPONGE));
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKENSPONGE.get(), SpawnPlacements.Type.ON_GROUND,
