...
public class AChickenBreezeEntity extends Chicken {
	public int eggTime = this.random.nextInt(6000) + 6000;

	/*public AChickenAcaciaWoodEntity(Level level) {
		this(ModEntities.A_CHICKEN_ACACIA_WOOD.get(), level);

	}

	public AChickenAcaciaWoodEntity(EntityType<? extends Chicken> type, Level worldIn) {
		super(type, worldIn);
	}*/

	public AChickenBreezeEntity(EntityType<AChickenBreezeEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	public Packet<ClientGamePacketListener> getAddEntityPacket() {
		...
		return sendPairingData(ServerPlayer, Consumer<CustomPacketPayload>);
	}*/
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Items.BREEZE_ROD));
	}
	...
	public void aiStep() {
		...
		if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && !this.isChickenJockey() && --this.eggTime <= 0) {
			...
			this.spawnAtLocation(Items.BREEZE_ROD);
