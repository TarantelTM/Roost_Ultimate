...
public class AChickenBasaltEntity extends Chicken {

	public int eggTime = this.random.nextInt(10) + 20;


	public AChickenBasaltEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKEN_BASALT.get(), world);
	}

	public AChickenBasaltEntity(EntityType<AChickenBasaltEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Items.BASALT));
	}
	@Override
	public void aiStep() {
		super.aiStep();
	...
	public static void init() {
		/*SpawnPlacements.register(ChickenRoostModEntities.A_CHICKEN_BASALT.get(), SpawnPlacements.Type.ON_GROUND,
				Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, oldmenu, reason, pos, random) -> (oldmenu.getDifficulty() != Difficulty.PEACEFUL
						&& Monster.isDarkEnoughToSpawn(oldmenu, pos, random) && Mob.checkMobSpawnRules(entityType, oldmenu, reason, pos, random)));
*/
	}
