...
public class AChickenSugarEntity extends Chicken {

	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("beach"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
	if((boolean) SpawnConfig.Spawn_Sugar.get() == true && (boolean) SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKEN_SUGAR.get(), SpawnConfig.Sugar_Spawnweight.get(), SpawnConfig.Sugar_MinGroupsize.get(), SpawnConfig.Sugar_MaxGroupsize.get()));
	}
	}


	public AChickenSugarEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKEN_SUGAR.get(), world);
	}

	public AChickenSugarEntity(EntityType<AChickenSugarEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Blocks.SUGAR_CANE));
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKEN_SUGAR.get(), SpawnPlacements.Type.ON_GROUND,
