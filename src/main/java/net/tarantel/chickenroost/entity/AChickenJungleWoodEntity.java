...
public class AChickenJungleWoodEntity extends Chicken {
	...
	if((boolean) SpawnConfig.Spawn_Jungle.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKEN_JUNGLE_WOOD.get(), SpawnConfig.Jungle_Spawnweight.get(), SpawnConfig.Jungle_MinGroupsize.get(), SpawnConfig.Jungle_MaxGroupsize.get()));
	}
	}


	public AChickenJungleWoodEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKEN_JUNGLE_WOOD.get(), world);
	}

	public AChickenJungleWoodEntity(EntityType<AChickenJungleWoodEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Blocks.JUNGLE_LOG));
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKEN_JUNGLE_WOOD.get(), SpawnPlacements.Type.ON_GROUND,
