...
public class AChickenappleEntity extends Chicken {
	...
	if((boolean) SpawnConfig.Spawn_Apple.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKENAPPLE.get(), SpawnConfig.Apple_Spawnweight.get(), SpawnConfig.Apple_MinGroupsize.get(), SpawnConfig.Apple_MaxGroupsize.get()));
	}
	}


	public AChickenappleEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKENAPPLE.get(), world);
	}

	public AChickenappleEntity(EntityType<AChickenappleEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKENAPPLE.get(), SpawnPlacements.Type.ON_GROUND,
