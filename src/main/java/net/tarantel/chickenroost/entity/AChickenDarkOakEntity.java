...
public class AChickenDarkOakEntity extends Chicken {

	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("dark_forest"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
	if((boolean) SpawnConfig.Spawn_DarkOak.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKEN_DARK_OAK.get(), SpawnConfig.DarkOak_Spawnweight.get(), SpawnConfig.DarkOak_MinGroupsize.get(), SpawnConfig.DarkOak_MaxGroupsize.get()));
	}
	}


	public AChickenDarkOakEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKEN_DARK_OAK.get(), world);
	}

	public AChickenDarkOakEntity(EntityType<AChickenDarkOakEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Blocks.DARK_OAK_LOG));
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKEN_DARK_OAK.get(), SpawnPlacements.Type.ON_GROUND,
