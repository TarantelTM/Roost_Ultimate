...
public class AChickenRabbitHideEntity extends Chicken {
	...
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("old_growth_birch_forest"), new ResourceLocation("forest"),
			new ResourceLocation("dark_forest"), new ResourceLocation("birch_forest"), new ResourceLocation("flower_forest"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
	if((boolean) SpawnConfig.Spawn_Rabbithide.get() == true && (boolean)SpawnConfig.Spawn_All.get() == true){
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ModEntities.A_CHICKEN_RABBIT_HIDE.get(), SpawnConfig.Rabbithide_Spawnweight.get(), SpawnConfig.Rabbithide_MinGroupsize.get(), SpawnConfig.Rabbithide_MaxGroupsize.get()));
	}
	}


	public AChickenRabbitHideEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ModEntities.A_CHICKEN_RABBIT_HIDE.get(), world);
	}

	public AChickenRabbitHideEntity(EntityType<AChickenRabbitHideEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(ModEntities.A_CHICKEN_RABBIT_HIDE.get(), SpawnPlacements.Type.ON_GROUND,
