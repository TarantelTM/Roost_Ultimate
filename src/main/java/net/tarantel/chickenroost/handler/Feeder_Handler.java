...
public class Feeder_Handler extends AbstractContainerMenu {
    ...
    public Feeder_Handler(int id, Inventory playerInv, FriendlyByteBuf buf) {
        this(id, playerInv, playerInv.player.level().getBlockEntity(buf.readBlockPos()));
    }

    public Feeder_Handler(int id, Inventory playerInventory, BlockEntity be) {
        super(ModHandlers.FEEDER_MENU.get(), id);
        this.level = playerInventory.player.level();
        this.blockEntity = be;

        if (be instanceof Feeder_Tile tile) {
