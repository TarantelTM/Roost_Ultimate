...
public class EmiRoostRecipe extends BasicEmiRecipe {
   public EmiRoostRecipe(RoostRecipe recipe) {
      super(EmiRoostPlugin.ROOST_CATEGORY, recipe.getId(), 100, 20);
      this.inputs.add(EmiIngredient.of((Ingredient)recipe.getIngredients().get(0)));
      this.inputs.add(EmiIngredient.of((Ingredient)recipe.getIngredients().get(1)));
   ...
   public void addWidgets(WidgetHolder widgets) {
      ...
      widgets.addSlot((EmiIngredient)this.inputs.get(1), 20, 0);
      widgets.addAnimatedTexture(EmiTexture.FULL_ARROW, 50, 0, 500, true, false, false);
      widgets.addSlot((EmiIngredient)this.outputs.getFirst(), 80, 0).recipeContext(this).getRecipe();
   }
}
