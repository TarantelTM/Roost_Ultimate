...
public class SoulExtractionRecipeCategory implements IRecipeCategory<SoulExtractorRecipe> {
   public static final ResourceLocation UID = ChickenRoostMod.ownresource("soul_extraction");
   ...
   public static final RecipeType<SoulExtractorRecipe> RECIPE_TYPE = RecipeType.create("chicken_roost", "soul_extraction", SoulExtractorRecipe.class);
   ...
   public SoulExtractionRecipeCategory(IGuiHelper helper) {
      ...
      this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack((ItemLike)ModBlocks.SOUL_EXTRACTOR.get()));
   ...
   public void draw(SoulExtractorRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
   ...
   public void setRecipe(@NotNull IRecipeLayoutBuilder builder, SoulExtractorRecipe recipe, @NotNull IFocusGroup focuses) {
      builder.addSlot(RecipeIngredientRole.INPUT, 25, 1).addIngredients((Ingredient)recipe.getIngredients().get(0));
