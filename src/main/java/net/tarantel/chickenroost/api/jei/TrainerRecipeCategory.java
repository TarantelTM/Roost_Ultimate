...
public class TrainerRecipeCategory implements IRecipeCategory<TrainerRecipe> {
   public static final ResourceLocation UID = ChickenRoostMod.ownresource("trainer_output");
   ...
   public static final RecipeType<TrainerRecipe> RECIPE_TYPE = RecipeType.create("chicken_roost", "trainer_output", TrainerRecipe.class);
   ...
   public TrainerRecipeCategory(IGuiHelper helper) {
      ...
      this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack((ItemLike)ModBlocks.TRAINER.get()));
   ...
   public void draw(TrainerRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
   ...
   public void setRecipe(@NotNull IRecipeLayoutBuilder builder, TrainerRecipe recipe, @NotNull IFocusGroup focuses) {
      builder.addSlot(RecipeIngredientRole.INPUT, 91, 1).addIngredients((Ingredient)recipe.getIngredients().get(0));
