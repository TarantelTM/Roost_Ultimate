...
public class SetFeederRoundRobinPayload {
    private final BlockPos feederPos;
    private final boolean enabled;

    public SetFeederRoundRobinPayload(BlockPos feederPos, boolean enabled) {
        this.feederPos = feederPos;
        this.enabled = enabled;
    }

    public SetFeederRoundRobinPayload(FriendlyByteBuf buf) {
    ...
    public void handle(Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (be instanceof Feeder_Tile feeder) {
                feeder.setRoundRobinEnabled(enabled); // ✅ dein Setter
            }
        });
